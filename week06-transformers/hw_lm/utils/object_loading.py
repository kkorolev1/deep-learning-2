...
import hw_lm.datasets
from hw_lm import batch_sampler as batch_sampler_module
    ...
    for split, params in configs["data"].items():
        ...
        for ds in params["datasets"]:
            datasets.append(configs.init_obj(
                ds, hw_lm.datasets, config_parser=configs))
        assert len(datasets)
        if len(datasets) > 1:
        ...
        dataloader = DataLoader(
            dataset, batch_size=bs, collate_fn=dataset.get_collate_fn(),
