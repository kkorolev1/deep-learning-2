...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        self.fpn_params = nn.ModuleDict({
            "p3_1x1": nn.Conv2d(in_channels=64, out_channels=out_channels, kernel_size=1), 
            "p4_1x1": nn.Conv2d(in_channels=160, out_channels=out_channels, kernel_size=1),
            "p5_1x1": nn.Conv2d(in_channels=400, out_channels=out_channels, kernel_size=1),
            "p3_3x3": nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1), 
            "p4_3x3": nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1),
            "p5_3x3": nn.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, padding=1)
        })
        pass
        ######################################################################
        #                            END OF YOUR CODE                        #
    ...
    def forward(self, images: torch.Tensor):
        ...
        fpn_feats["p5"] = self.fpn_params["p5_1x1"](backbone_feats["c5"])
        fpn_feats["p4"] = F.interpolate(fpn_feats["p5"], scale_factor=self.fpn_strides["p5"] / self.fpn_strides["p4"]) + self.fpn_params["p4_1x1"](backbone_feats["c4"])
        fpn_feats["p3"] = F.interpolate(fpn_feats["p4"], scale_factor=self.fpn_strides["p4"] / self.fpn_strides["p3"]) + self.fpn_params["p3_1x1"](backbone_feats["c3"])
        
        fpn_feats["p5"] = self.fpn_params["p5_3x3"](fpn_feats["p5"])
        fpn_feats["p4"] = self.fpn_params["p4_3x3"](fpn_feats["p4"])
        fpn_feats["p3"] = self.fpn_params["p3_3x3"](fpn_feats["p3"])
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        # Replace "pass" statement with your code
        h_coords = torch.arange(feat_shape[2], dtype=dtype, device=device)
        w_coords = torch.arange(feat_shape[3], dtype=dtype, device=device)
        stride = strides_per_fpn_level[level_name]
        hc, wc = stride * (h_coords + 0.5), stride * (w_coords + 0.5)
        HC, WC = torch.meshgrid(hc, wc, indexing='ij')
        location_coords[level_name] = torch.flip(torch.stack((HC, WC), dim=-1).reshape(-1, 2), dims=(1,))
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # Replace "pass" statement with your code
        stem_cls = [
            nn.Conv2d(in_channels=in_channels, out_channels=stem_channels[0], kernel_size=3, padding=1),
            nn.ReLU(),
            #nn.BatchNorm2d(stem_channels[0]),
            nn.Conv2d(in_channels=in_channels, out_channels=stem_channels[0], kernel_size=3, padding=1),
            nn.ReLU(),
            #nn.BatchNorm2d(stem_channels[0]),
            nn.Conv2d(in_channels=in_channels, out_channels=stem_channels[0], kernel_size=3, padding=1),
        ...
        stem_box = [
            nn.Conv2d(in_channels=in_channels, out_channels=stem_channels[1], kernel_size=3, padding=1),
            nn.ReLU(),
        ...
        # Class conv
        self.pred_cls = nn.Conv2d(in_channels=stem_channels[0], out_channels=num_classes, kernel_size=3, padding=1)
        
        # Box regression conv
        self.pred_box = nn.Conv2d(in_channels=stem_channels[1], out_channels=4, kernel_size=3, padding=1) 

        # Centersness conv
        self.pred_ctr = nn.Conv2d(in_channels=stem_channels[0], out_channels=1, kernel_size=3, padding=1)  
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, fpn_level in feats_per_fpn_level.items():
            ...
            class_logits[key] = torch.flatten(self.pred_cls(cls_out), -2, -1).transpose(1, 2)
            boxreg_deltas[key] = torch.flatten(self.pred_box(box_out), -2, -1).transpose(1, 2)
            centerness_logits[key] = torch.flatten(self.pred_ctr(box_out), -2, -1).transpose(1, 2)
...
def fcos_apply_deltas_to_locations(
    ...
    left, top, right, bottom = deltas[..., :4].unbind(dim=-1)
    xc, yc = locations[..., :2].unbind(dim=-1)
    ...
    output_boxes[bg_deltas] = torch.stack((xc, yc, xc, yc), dim=-1)[bg_deltas]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    left, top, right, bottom = deltas[..., :4].unbind(dim=-1)
    bg_deltas = left == -1
    centerness = torch.sqrt(torch.min(left, right) / torch.max(left, right) * torch.min(top, bottom) / torch.max(top, bottom))
    centerness[bg_deltas] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def iou(box1: torch.Tensor, box2: torch.Tensor):
    left1, top1, right1, bottom1 = box1.unbind()
    left2, top2, right2, bottom2 = box2.unbind()
    ...
    intersec = box_square(left, top, right, bottom)
    union = box_square(left1, top1, right1, bottom1) + box_square(left2, top2, right2, bottom2) - intersec
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    #############################################################################
    # Replace "pass" statement with your code
    for box_index in torch.argsort(scores, descending=True):
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for gt_boxes_per_image in gt_boxes:
            matched_gt_boxes.append(
                fcos_match_locations_to_gt(
                    locations_per_fpn_level, strides_per_fpn_level, gt_boxes_per_image
                )
            )
        ...
        for gt_boxes_dict in matched_gt_boxes:
            ...
            for level_name, gt_boxes_on_level in gt_boxes_dict.items():
                matched_gt_deltas_dict[level_name] = fcos_get_deltas_from_locations(
        ...
        one_hot_gt = F.one_hot(matched_gt_boxes[:, :, -1].to(torch.int64) + 1, num_classes=self.num_classes + 1)
        one_hot_gt = one_hot_gt[:, :, 1:].to(torch.float32) # remove bg class index
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 2:
            # Replace "pass" statement with your code
            scores_mask = level_pred_scores > test_score_thresh
            level_pred_classes = level_pred_classes[scores_mask]
            level_pred_scores = level_pred_scores[scores_mask]
            ...
            level_pred_boxes = level_pred_boxes[scores_mask]

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            level_pred_boxes[:, 0].clip_(0, images[0].shape[-1])
            level_pred_boxes[:, 1].clip_(0, images[0].shape[-2])
            level_pred_boxes[:, 2].clip_(0, images[0].shape[-1])
            level_pred_boxes[:, 3].clip_(0, images[0].shape[-2])
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
